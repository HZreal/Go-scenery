...
// First 返回列表链表头结点
func (list *DoublyLinkedList) First() *ListNode {
	return list.head
}

// Last 返回列表链表尾结点
func (list *DoublyLinkedList) Last() *ListNode {
	return list.tail
}

// AddNodeFromHead 从头部开始，添加节点到第N+1个元素之前，N=0表示添加到第一个元素之前，表示新节点成为新的头部，N=1表示添加到第二个元素之前，以此类推
func (list *DoublyLinkedList) AddNodeFromHead(n int, v string) {
	// 加并发锁
	list.lock.Lock()
...
func (list *DoublyLinkedList) AddNodeFromTail(n int, v string) {
	// 加并发锁
	list.lock.Lock()
	defer list.lock.Unlock()
...
func (list *DoublyLinkedList) IndexFromHead(n int) *ListNode {
...
func (list *DoublyLinkedList) IndexFromTail(n int) *ListNode {
	// 索引超过或等于列表长度，一定找不到，返回空指针
	if n >= list.len {
		return nil
	}
...
func (list *DoublyLinkedList) PopFromHead(n int) *ListNode {
	// 加并发锁
	list.lock.Lock()
	defer list.lock.Unlock()
...
func (list *DoublyLinkedList) PopFromTail(n int) *ListNode {
	// 加并发锁
	list.lock.Lock()
	defer list.lock.Unlock()
...
func main() {
	...
	list.AddNodeFromHead(0, "are")
	list.AddNodeFromHead(0, "happy")
	// 在列表尾部插入新元素
	list.AddNodeFromTail(0, "11")
	list.AddNodeFromTail(0, "22")

	list.AddNodeFromTail(list.Len()-1, "333")
	list.AddNodeFromHead(list.Len()-1, "444")
